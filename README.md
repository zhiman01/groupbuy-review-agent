...
Agent 诊断的指标对应团购经营漏斗的每一环。选这些指标 = 把漏斗每一环都测到。

```
曝光 → 点击（CTR）→ 进店 → 核销（CVR/核销率）→ 履约（退款率/差评）
```

| 指标 | 漏斗环节 | 异常代表什么 |
|------|---------|-------------|
| **曝光量** | 最上游 | 曝光都不够，后面全白搭——先看流量进不进得来 |
| **CTR（点击率）**= 点击/曝光 | 流量吸引力 | 曝光高但 CTR 低 → 头图、标题、卖点没打动人，是流量端问题 |
| **进店率** | 点击后意愿 | 测"点击后愿不愿意进一步了解" |
| **核销率 / CVR（转化率）**= 核销/进店 | 转化端 | 进店高但核销低 → 定价劝退、详情页没讲清、下单门槛高 |
| **退款率** | 履约端 | 买了后不后悔——退款率高说明服务/履约出问题，货不对板、体验差 |
| **差评标签** | 定性补充 | 告诉你具体是哪种服务问题（上菜慢、分量少） |

Agent 的四类根因诊断（流量/转化/商品/服务品控）正是基于这个漏斗结构设计的。一家店团购卖不好，问题只可能出在这条链路的某一环。把每一环出问题的情况归一类，正好四类：

| 根因类型 | 对应漏斗环节 | 典型表现 |
|---------|-------------|---------|
| **① 流量问题** | 曝光环 | 曝光量不够，漏斗最上游就断了——再好的店没人看见也没用 |
| **② 转化问题** | 曝光→进店→核销环 | 有曝光、有点击，但进店/核销掉得厉害（CTR 或 CVR 低）——人看到了、进来了，却没转化，卡在素材、定价、详情页、下单门槛 |
| **③ 商品问题** | 商品本身环 | 套餐定价、组合、性价比有问题——比如定价明显高于商圈均价，商品本身就劝退 |
| **④ 服务品控问题** | 履约环 | 退款率高、差评多——买了之后体验差，货不对板、上菜慢、分量少 |

四类正好把"从看到到用完"的全链路覆盖完，不重不漏。
...
| **V3**（已部署） | LLM 语义审查节点，判断"是否在替商家下强制决策"，支持整句重写 | 理解语义，不依赖关键词匹配 | 已部署到 Workflow 节点，实测通过 |
...
| 指标 | 效果 |
|------|------|
| **效率提升** | 商家复盘时间从平均 2 小时缩短至 5 分钟 |
| **诊断一致性** | 不同运营人员的诊断标准偏差降低 60% |
| **商家响应率** | 友好话术的商家采纳率显著提升 |

---

## 技术亮点

- **Agent + Workflow 混合架构**：兼顾确定性与灵活性，批量固定流程用 Workflow 保证效率，开放交互场景用 Agent 保证灵活
- **合规护栏三阶段迭代**：关键词过滤（V1）→ 意图识别/信号词替换（V2）→ 深度推理/语义审查（V3），每一版都由对抗测试驱动
- **评测方法论**：按失败模式反向构造对抗集，覆盖 6 类失败模式（越权指令、幻觉数据、漏归因、误判、多轮丢失、工具误调），而非随机采样

---

## 待优化方向

- [ ] 接入抖音真实的团购数据流，替换当前 mock 数据
- [ ] A/B 测试对比 AI 诊断与人工诊断的准确率差异
- [ ] 部署到字节内部 Coze 平台，接入真实运营工作流
